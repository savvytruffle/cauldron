...
nbody_rk.argtypes = [ndpointer(dtype=ctypes.c_double), ndpointer(dtype=ctypes.c_double),
                       ndpointer(dtype=ctypes.c_double), ctypes.c_double, ctypes.c_int, ctypes.c_int, ctypes.c_int]
...
def nbody_c(r_i, v_i, mass, dt, Nsteps, Nbody=3, Ndim=3, downsample=1):
	...
	r = r.flatten()
	v = v.flatten()
	nbody_rk(r, v, mass, dt, Nbody, Nsteps, downsample)
	return r, v
...
def sim_lc(r_i, v_i, mass, r1, r2, rp, dt, Nbody, Ndim, Nsteps, downsample, frat, u1, u2, u3, u4):
	#r, v in sky/obs plane 
	t = np.arange(Nsteps/downsample)*dt
	sky_r, sky_v = nbody_c(r_i, v_i, mass, dt, Nsteps, Nbody=Nbody, downsample=downsample)
	f = np.ones(len(t))
	f2 = f*1.0
	...
	if eclipse32.sum() > 0:
		...
		f2[eclipse32] *= (_f + 1./frat) / (1. + 1./frat)
		#plt.plot(t[eclipse32], _f, 'b-')
		#plt.plot(t[eclipse32], f2[eclipse32], 'c-')
		#plt.show()

	return t, f, f2, sky_r, sky_v, z_bin, z_p1, z_p2
...
ti = 0.
dt = 0.05 #units of hr #kepler short cadence = 0.002 d
Nbody = 3 #5
Ndim = 3
Nsteps = int(4*365.*24/dt)#800000 
downsample = 1
