'''This program plots an HR diagram with isochrones for a single binary star system, in 
   this case KIC 6131659. It prints out a calculated temperature ratio and teff for the 
...
starId = 6131659                                        # Kepler Input Catalog #
aspcapTeff = 4845                                       # Teff of starId from ASPCAP
teff1err = 98                                           # Teff (primary) error from ASPCAP
logTeff1err = [1.991226]                                # error on the above
logTeff2err = [1.504981]                                # log(err) on Teff2 which is calculated below
BFfluxRatio = [0.6478124125]                            # flux ratios from BF areas
BFfluxRatioErr = [0.02824]                              # error on the above
KEBLATfluxratio = [0.1483]                              # KEBLAT flux ratio *NOT A SURFACE BRIGHTNESS*
KEBLATfluxratio_err = [0.002]                           # error on the above
KEBLATradiusratio = 0.6799                              # KEBLAT radius ratio
KEBLATradiusratio_err = 0.0057                          # error on the above
logg1 = [4.813282]                                      # log(g) of star 1 from BF flux ratio
logg1_err = [0.05458]                                   # error on the above
logg2 = [4.457921]                                      # log(g) of star 2 from BF flux ratio
logg2_err = [0.038261]                                  # error on the above 
...
#### Now we want to find the location at which the flux ratio matches the isochroneSb / sb1. ####
    ...
    KEBLATsbratio = np.array(KEBLATfluxratio) / np.array(KEBLATradiusratio**2)
    KEBLATsbratio_err = np.array(KEBLATsbratio) * (np.sqrt((np.array(KEBLATfluxratio_err) / np.array(KEBLATfluxratio))**2 + (np.array(KEBLATradiusratio) / np.array(KEBLATradiusratio_err))**2))
    fit = np.argmin(np.abs(isochroneSb/sb1 - KEBLATsbratio)) #### Using the KEBLAT SB Ratio calculated above
...
#### Attempting some imprecise error propagation #### 
    KEBLATsbratiomax = np.array(KEBLATsbratio) + np.array(KEBLATsbratio_err)
    KEBLATsbratiomin = np.array(KEBLATsbratio) - np.array(KEBLATsbratio_err)
    fitmax = np.argmax(np.abs(isochroneSb/sb1 - KEBLATsbratiomax))
...
plt.errorbar(np.log10(teff2s[0]), logg2, yerr=logg2_err, xerr=Teff2err, color='C9', ls='None', marker='o', label='Secondary')
...
plt.title(starId)
plt.legend()
plt.savefig('figures/HRDiagrams/6131659_1.png')
plt.savefig('figures/HRDiagrams/6131659_1.jpeg')
plt.show()