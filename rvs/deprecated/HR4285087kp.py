'''This program plots an HR diagram with isochrones for a single binary star system, in 
   this case KIC 4285087. It prints out a calculated temperature ratio and teff for the 
...
starId = 4285087                                        # Kepler Input Catalog #
aspcapTeff = 5664                                       # Teff of starId from ASPCAP
teff1err = 146                                          # Teff (primary) error from ASPCAP
logTeff1err = [2.164353]                                # error on the above
logTeff2err = [1.447506]                                # log(err) on Teff2 which is calculated below
BFfluxRatio = [0.9966328203]                            # flux ratios from BF areas
BFfluxRatioErr = [0.02241]                              # error on the above
KEBLATfluxratio = [0.901]                               # KEBLAT flux ratio *NOT A SURFACE BRIGHTNESS*
KEBLATfluxratio_err = [0.08]                            # error on the above
KEBLATradiusratio = 0.969                               # KEBLAT radius ratio
KEBLATradiusratio_err = 0.0080                          # error on the above
logg1 = [4.487564]                                      # log(g) of star 1 from BF flux ratio
logg1_err = [0.021908]                                  # error on the above
logg2 = [4.457921]                                      # log(g) of star 2 from BF flux ratio
logg2_err = [0.020566]                                  # error on the above 
...
plt.title(starId)
plt.legend()
plt.savefig('figures/HRDiagrams/4285087_1.png')
plt.savefig('figures/HRDiagrams/4285087_1.jpeg')
plt.show()