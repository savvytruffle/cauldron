...
teff1err = 162                                          # Teff (primary) error from ASPCAP
logTeff1err = [2.209515]                                # error on the above
logTeff2err = [1.886255]                                # log(err) on Teff2 which is calculated below
BFfluxRatio = [0.4621566737]                            # flux ratios from BF areas
BFfluxRatioErr = [0.02931]                              # error on the above
KEBLATfluxratio = [0.19138]                             # KEBLAT flux ratio *NOT A SURFACE BRIGHTNESS*
KEBLATfluxratio_err = [0.00003]                         # error on the above
KEBLATradiusratio = 0.57093                             # KEBLAT radius ratio
KEBLATradiusratio_err = 0.013                           # error on the above
...
plt.errorbar(np.log10(aspcapTeff), logg2, yerr=logg2_err, xerr=Teff2err, color='C9', ls='None', marker='o', label='Secondary')
...
plt.title(starId)
plt.legend()
plt.savefig('figures/HRDiagrams/6778289_1.png')
plt.savefig('figures/HRDiagrams/6778289_1.jpeg')
plt.show()