...
def isochroneParse(isoDir, isoFile, loggMin=3.9):
    ...
    '''
    iBFFluxRatSoPath = os.path.join(isoDir, isoFile)
    isoMass, isoLogTeff, isoLogg = np.loadtxt(iBFFluxRatSoPath, unpack=True, usecols = (1,2,3))
...
##########################################################################################
    ...
    BFFluxRatSop_err = BFFluxRatSoP * np.sqrt((P_stderror/np.mean(PAreas))**2 + (S_stderror/np.mean(SAreas))**2)
    ...
    print('BF Flux Ratio (2/1) = {0:.3f} +/- {1:.3f}'.format(BFFluxRatSoP, BFFluxRatSop_err))
...
##########################################################################################
    ...
    Flux2_err = Flux2.cgs * np.sqrt((KEBLATFluxsum_err/KEBLATFluxsum)**2 +
                                                (Flux1_err/Flux1)**2)
    F2overF1 = Flux2/Flux1
    F2overF1_err = F2overF1 * np.sqrt((Flux1_err.cgs.value/Flux1.cgs.value)**2 + 
                                      (Flux2_err.cgs.value/Flux2.cgs.value)**2)

    print('R1 = {0:.3f} +/- {1:.3f}'.format(R1, R1_err))
    print('R2 = {0:.3f} +/- {1:.3f}'.format(R2, R2_err))
    print('R2/R1 = {0:.3f} +/- {1:.3f}'.format(R2oR1, R2oR1_err))
    print('Flux1 = {0:.3e} +/-  {1:.3e}'.format(Flux1.cgs, Flux1_err.cgs))
    print('Flux2 = {0:.3e} +/- {1:.3e}'.format(Flux2.cgs, Flux2_err.cgs))
