...
for i in range(1, len(bffitlist)):
    ...
    rvraw2.append(bffitlist[i][0][4]) # [4,5,6] = [amp, rv, width] for star 2;
    rvraw3.append(bffitlist[i][0][8]) # [8,9,10] = [amp, rv, width] for star 3;
    rvraw1_err.append(bffitlist[i][2][1])
    rvraw2_err.append(bffitlist[i][2][4])
    rvraw3_err.append(bffitlist[i][2][7]) #Should it be 7, or 8??? What even is life?

# CALCULATE ORBITAL PHASES AND FINAL RV CURVE
rvdata = bff.rvphasecalc(bjdinfile, bjdoffset, nspec, period, BJD0, rvraw1, rvraw1_err, rvraw2, rvraw2_err, rvraw3, rvraw3_err, rvstd, bcvstd)
phase = rvdata[0]; bjdfunny = rvdata[1]
rv1 = rvdata[2]; rv2 = rvdata[3]; rv3 = rvdata[4]
rv1_err = rvdata[4]; rv2_err = rvdata[5]; rv3_err = rvdata[6]
g2 = open(outfile, 'w')
...
print('#', file=g2)
print('# time, phase, adjusted_time, RV1 [km/s], error1 [km/s], RV2 [km/s], error2 [km/s], RV3 [km/s], error3 [km/s]', file=g2)
print('#', file=g2)
for i in range(1, nspec):
    print ('%.9f %.9f %.9f %.5f %.5f %.5f %.5f' % (bjdfunny[i] + bjdoffset, phase[i], bjdfunny[i], 
            rv1[i], rv1_err[i], rv2[i], rv2_err[i], rv3[i], rv3_err[i]), file=g2)
g2.close()
...
try:
    ...
    for idx in range(1, nspec):
        ...
        print('# Gaussian 3 [amp, RV +/- err, wid]: [{0:.2f}, {1:.2f} +/- {2:.2f}, {3:.2f}]'.format(bffitlist[i][0][6], rvraw3[i], rvraw3_err[i], bffitlist[i][0][8]), file=bfout)
...
for i in range (1, nspec):
    ...
    gauss3 = gaussian(bf_ind, bffitlist[i][0][6], bffitlist[i][0][7], bffitlist[i][0][8])
